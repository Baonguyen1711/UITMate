  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Chào mừng bạn đến với UITMate",
      });
      navigate("/");
    }, 1000);
  };
  ...
  const features = [
    {
      ...
      desc: "Quản lý lịch học thông minh",
    },
    {
      icon: GraduationCap,
      title: "Theo dõi điểm",
      desc: "Cập nhật kết quả học tập",
    },
    {
      icon: MessageSquare,
      title: "UITThread",
      desc: "Kết nối cộng đồng sinh viên",
    },
    { icon: Users, title: "Nhóm đồ án", desc: "Làm việc nhóm hiệu quả" },
  ];
      ...
      <div className="hidden lg:flex lg:w-1/2 gradient-hero p-12 flex-col justify-between">
        <div>
          ...
          <h1 className="text-4xl font-bold text-primary-foreground mb-4 animate-slide-up">
            Trung tâm học tập
            <br />& kết nối sinh viên
          </h1>
          <p
            className="text-lg text-primary-foreground/80 max-w-md animate-slide-up"
            style={{ animationDelay: "0.1s" }}
          >
            Quản lý thông tin học tập, theo dõi kết quả và gắn kết cộng đồng
        ...
        <div className="grid grid-cols-2 gap-4">
          {features.map((feature, index) => (
            <div
              key={feature.title}
              className="bg-primary-foreground/10 backdrop-blur rounded-xl p-4 animate-slide-up"
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <Card className="w-full max-w-md shadow-xl border-0 animate-scale-in">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Đăng nhập để tiếp tục sử dụng UITMate
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="email" className="w-full">
              <TabsList className="grid w-full grid-cols-2 mb-6">
                <TabsTrigger value="email">Email UIT</TabsTrigger>
              ...
              <TabsContent value="google">
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground text-center">
                    Đăng nhập nhanh bằng tài khoản Google của trường
                  </p>
                  <Button
                    variant="outline"
                    className="w-full"
